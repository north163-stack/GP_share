...
# # 【核心新增】：使用 Streamlit 云端缓存包裹 AI 调用，解决 429 报错与重复耗费 Token 问题
# @st.cache_data(show_spinner=False)
# def generate_cached_report(_model, data_fingerprint, df):
#     # _model 加下划线防报错；data_fingerprint 用于判断源数据是否更新
#     return get_zeus_style_insight(_model, df)
...
#         # --- 模块 3: 🤖 AI 深度洞察与战略规划 (云端缓存版) ---
#         st.markdown('<div class="section-title">🤖 AI 深度洞察与战略建议</div>', unsafe_allow_html=True)
        
#         # 1. 提取当前数据源的“指纹”
#         current_data_fingerprint = f"{len(df)}_{df['date'].max()}"
        
#         # 2. 按钮布局
#         btn_col1, btn_col2 = st.columns([2, 8])
#         with btn_col1:
#             generate_clicked = st.button("⚡ 立即生成报告", type="primary")
#         with btn_col2:
#             if st.button("🗑️ 强制清除云端缓存"):
#                 st.cache_data.clear()
#                 st.rerun()
                
#         # 3. 使用 session_state 仅控制当前页面的显示状态
#         if "ai_report_display" not in st.session_state:
#             st.session_state.ai_report_display = None
...
#                 st.error("未配置 API Key，无法呼叫 AI 大模型。")
#             else:
#                 with st.spinner('AI 正在交叉验证数据，生成高管级汇报纪要... (若数据未变将秒读缓存)'):
#                     # 触发带有全局缓存机制的函数
#                     report_content = generate_cached_report(gemini_model, current_data_fingerprint, df)
...
#             )
#         else:
#             st.info("👆 点击上方按钮生成诊断报告。生成后将自动缓存于云端，页面刷新不再消耗 API。")
